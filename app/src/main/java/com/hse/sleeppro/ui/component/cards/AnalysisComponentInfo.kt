...
@Composable
fun AnalysisInfo(state: CardState) {
    when(state){
        CardState.ACTIVE -> AnalysisInfoActive()
        CardState.PASSIVE -> AnalysisInfoPassive()
    }
}
@Composable
fun AnalysisInfoActive(){
    Box(modifier =Modifier.fillMaxWidth(), contentAlignment = Alignment.Center) {
        Card(shape = RoundedCornerShape(15.dp), elevation = 15.dp) {
            Row() {
                Image(
                    painter = painterResource(id = R.drawable.sine_wave),
